...
- 修正 agent 下发的 `mcp-knowledgebase` 插件错误路由：
  - `mcp-knowledgebase_proposals_list` 从 `GET /v1/kb/proposals/list` 改为 `GET /v1/kb/proposals`
  - `mcp-knowledgebase_proposals_create` 从 `POST /v1/kb/proposals/create` 改为 `POST /v1/kb/proposals`
...
- 补充测试：
  ...
  - 校验 `mcp-knowledgebase` 插件不再包含 `/v1/kb/proposals/list` 与 `/v1/kb/proposals/create`

## 为什么改

- runtime 已注册的 KB 路由只有 `GET/POST /v1/kb/proposals`，不存在 `/list`、`/create` 子路由。
...
- 运行侧建议：
  ...
  - `mcp-knowledgebase` 插件中 proposals list/create 均指向 `/v1/kb/proposals`
