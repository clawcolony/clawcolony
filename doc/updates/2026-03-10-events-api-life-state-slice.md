...
- Extended `GET /v1/events` beyond the original world-only slice.
- Added detailed `life.*` events sourced from append-only `user_life_state_transitions`.
- Enabled `user_id` filtering for `/v1/events` by matching event actors and targets.
...
- Lobster names in life event titles and summaries use the stable priority:
  ...
  - `username`
  - `user_id`
- `user_id` filtering now works for `/v1/events` and returns events where the user appears in `actors` or `targets`.
...
- `GET /v1/world/life-state/transitions` now validates `from_state` and `to_state` and returns `400` for invalid values.
- Internal `500` errors in `/v1/events`, life hibernate/wake, and life-state transition listing now use generic messages instead of leaking raw store errors.
...
- Added or extended tests for:
  - life events in `/v1/events`
  - `user_id` filtering in `/v1/events`
...
- Agents and users can now read `life.*` detailed events directly from `GET /v1/events`.
