...
func (s *InMemoryStore) GetBotCredentials(_ context.Context, userID string) (BotCredentials, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return BotCredentials{
		...
		UpgradeToken: b.UpgradeToken,
	}, nil
}

func (s *InMemoryStore) UpsertBotCredentials(_ context.Context, creds BotCredentials) (BotCredentials, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	s.ensureBot(uid)
	b := s.bots[uid]
	if v := strings.TrimSpace(creds.GatewayToken); v != "" {
		b.GatewayToken = v
	}
	if v := strings.TrimSpace(creds.UpgradeToken); v != "" {
		b.UpgradeToken = v
	}
...
func (s *InMemoryStore) CreateUpgradeAudit(_ context.Context, item UpgradeAudit) (UpgradeAudit, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.nextUpgradeID++
	item.ID = s.nextUpgradeID
	if item.StartedAt.IsZero() {
		item.StartedAt = time.Now().UTC()
	}
	s.upgrades = append(s.upgrades, item)
	return item, nil
}

func (s *InMemoryStore) AppendUpgradeStep(_ context.Context, step UpgradeStep) (UpgradeStep, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.nextStepID++
	...
	s.upgradeSteps = append(s.upgradeSteps, step)
	return step, nil
}

func (s *InMemoryStore) FinishUpgradeAudit(_ context.Context, id int64, status, image, errorText string, finishedAt time.Time) (UpgradeAudit, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for i, it := range s.upgrades {
		if it.ID != id {
			continue
		}
		it.Status = status
		...
		if !it.StartedAt.IsZero() && !finishedAt.IsZero() {
			it.DurationMS = finishedAt.Sub(it.StartedAt).Milliseconds()
		}
		s.upgrades[i] = it
		return it, nil
	}
	return UpgradeAudit{}, fmt.Errorf("upgrade audit not found: %d", id)
}

func (s *InMemoryStore) GetUpgradeAudit(_ context.Context, id int64) (UpgradeAudit, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for _, it := range s.upgrades {
		if it.ID == id {
			return it, nil
		}
	}
...
func (s *InMemoryStore) ListUpgradeAudits(_ context.Context, userID string, limit int) ([]UpgradeAudit, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]UpgradeAudit, 0, len(s.upgrades))
	for _, it := range s.upgrades {
		if userID != "" && it.UserID != userID {
			continue
		}
...
func (s *InMemoryStore) ListUpgradeSteps(_ context.Context, auditID int64, limit int) ([]UpgradeStep, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]UpgradeStep, 0)
	for _, it := range s.upgradeSteps {
		if it.AuditID != auditID {
			continue
		}
		out = append(out, it)
	}
...
func (s *InMemoryStore) CreateRegisterTask(_ context.Context, item RegisterTask) (RegisterTask, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.nextRegisterTaskID++
	item.ID = s.nextRegisterTaskID
	if item.StartedAt.IsZero() {
		item.StartedAt = time.Now().UTC()
	}
	...
	s.registerTasks = append(s.registerTasks, item)
	return item, nil
}

func (s *InMemoryStore) AppendRegisterTaskStep(_ context.Context, step RegisterTaskStep) (RegisterTaskStep, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.nextRegisterStepID++
	step.ID = s.nextRegisterStepID
	if step.CreatedAt.IsZero() {
		step.CreatedAt = time.Now().UTC()
	}
	s.registerSteps = append(s.registerSteps, step)
	return step, nil
}

func (s *InMemoryStore) FinishRegisterTask(_ context.Context, id int64, status, userID, userName, repoFullName, image, errorText string, finishedAt time.Time) (RegisterTask, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for i, it := range s.registerTasks {
		if it.ID != id {
			continue
		}
		...
		if strings.TrimSpace(userID) != "" {
			it.UserID = strings.TrimSpace(userID)
		}
		if strings.TrimSpace(userName) != "" {
			it.UserName = strings.TrimSpace(userName)
		}
		if strings.TrimSpace(repoFullName) != "" {
			it.RepoFullName = strings.TrimSpace(repoFullName)
		}
		if strings.TrimSpace(image) != "" {
			it.Image = strings.TrimSpace(image)
		}
		it.Error = strings.TrimSpace(errorText)
		it.FinishedAt = &finishedAt
		if !it.StartedAt.IsZero() && !finishedAt.IsZero() {
			it.DurationMS = finishedAt.Sub(it.StartedAt).Milliseconds()
		}
		s.registerTasks[i] = it
		return it, nil
	}
	return RegisterTask{}, fmt.Errorf("register task not found: %d", id)
}

func (s *InMemoryStore) GetRegisterTask(_ context.Context, id int64) (RegisterTask, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for _, it := range s.registerTasks {
		if it.ID == id {
			return it, nil
		}
	}
...
func (s *InMemoryStore) ListRegisterTasks(_ context.Context, limit int) ([]RegisterTask, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]RegisterTask, len(s.registerTasks))
	copy(out, s.registerTasks)
	sort.Slice(out, func(i, j int) bool {
		if out[i].StartedAt.Equal(out[j].StartedAt) {
			return out[i].ID > out[j].ID
		}
...
func (s *InMemoryStore) ListRegisterTaskSteps(_ context.Context, taskID int64, limit int) ([]RegisterTaskStep, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]RegisterTaskStep, 0)
	for _, it := range s.registerSteps {
		if it.TaskID != taskID {
			continue
		}
		out = append(out, it)
...
func (s *InMemoryStore) AppendChatMessage(_ context.Context, msg ChatMessage) (ChatMessage, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.nextChatID++
	msg.ID = s.nextChatID
...
func (s *InMemoryStore) ListChatMessages(_ context.Context, userID string, limit int) ([]ChatMessage, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if limit <= 0 {
		limit = 300
...
func (s *InMemoryStore) ListPromptTemplates(_ context.Context) ([]PromptTemplate, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	items := make([]PromptTemplate, 0, len(s.prompts))
	for _, it := range s.prompts {
...
func (s *InMemoryStore) UpsertPromptTemplate(_ context.Context, item PromptTemplate) (PromptTemplate, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	item.Key = strings.TrimSpace(item.Key)
	if item.Key == "" {
