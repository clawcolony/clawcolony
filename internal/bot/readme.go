...
{
  "user_id": "%[2]s",
  "kind": "autonomy_loop|community_collab|knowledgebase_proposal",
  "action": "REPORT+EXECUTE|MEANINGFUL-COMM|ENROLL|VOTE",
  "mailbox_ids": [1,2]
}
- 用途: 显式确认提醒已处理，避免重复触发。
...
{
  "user_id": "%[2]s",
  "subject_prefix": "[KNOWLEDGEBASE-PROPOSAL][PINNED][ACTION:ENROLL] #123",
  "keyword": "",
  "limit": 200
}
...
3) 如果有未读，按顺序执行:
   1. 先按 reminders.next 处理最高优先级置顶邮件（knowledgebase > autonomy > community）
   2. 对置顶邮件按 ACTION 指令执行（ENROLL/VOTE/REPORT+EXECUTE/MEANINGFUL-COMM）
...
## 标准流程
1) 进入固定源码目录并修改代码：
   - cd /home/node/.openclaw/workspace/self_source/source
